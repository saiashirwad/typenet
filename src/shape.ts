...
type NumAdd<A extends number, B extends number> = number extends A
  ? number
  : number extends B
    ? number
...
type NumMul<A extends number, B extends number> = number extends A
  ? number
  : number extends B
    ? number
...
type NumDiv<A extends number, B extends number> = number extends A
  ? number
  : number extends B
    ? number
...
export type Reverse<T extends any[], Acc extends any[] = []> = T extends [infer H, ...infer R]
  ? Reverse<R, [H, ...Acc]>
  : Acc;
...
type HasDup<T extends number[], Seen extends number = never> = T extends [
...
export type NormalizeDim<S extends Shape, D extends number> = number extends D
  ? number
...
export type IsValidDim<S extends Shape, D extends number> =
  IsDynamic<S> extends true
    ...
    : number extends D
      ? true
      : `${NormalizeDim<S, D>}` extends keyof S
        ? true
        : false;
...
type RemoveAt<S extends Shape, I extends number, Acc extends Shape = []> = S extends [
  infer X extends number,
  ...infer Xs extends number[],
]
...
type BroadcastDim<X extends number, Y extends number> =
  IsExact<X, Y> extends true
    ? X
    : IsExact<X, 1> extends true
      ? Y
      : IsExact<Y, 1> extends true
        ? X
        : IsExact<X, number> extends true
          ? Y
          : IsExact<Y, number> extends true
            ? X
            : X extends Y
              ? X
              : never;
...
type CanBroadcastRev<A extends Shape, B extends Shape> = A extends [
  infer X extends number,
  ...infer Xs extends number[],
]
...
type BroadcastRev<A extends Shape, B extends Shape, Acc extends Shape = []> = A extends [
  infer X extends number,
  ...infer Xs extends number[],
]
  ? B extends [infer Y extends number, ...infer Ys extends number[]]
...
export type Broadcast<A extends Shape, B extends Shape> =
  ...
  IsExact<A, B> extends true
    ? A
    : IsDynamic<A> extends true
      ? number[]
      : IsDynamic<B> extends true
        ? number[]
...
export type BroadcastCheck<A extends Shape, B extends Shape> =
  CanBroadcast<A, B> extends true
    ? unknown
        ...
        : B extends []
          ...
          : DimEq<InnerA<A>, InnerB<B>> extends true
            ? A["length"] extends 1
              ...
              : B["length"] extends 1
                ? unknown
                : CanBroadcast<BatchDims<A>, BatchDims<B>> extends true
                  ? unknown
...
type ProductSkipNegOne<V extends number[], Acc extends number = 1> = V extends [
  infer X extends number,
  ...infer Xs extends number[],
]
  ? X extends -1
...
type CountNegOnes<V extends number[], Acc extends 1[] = []> = V extends [
  infer X,
  ...infer Xs extends number[],
]
  ? X extends -1
...
export type ViewCheck<S extends Shape, V extends number[]> =
  CountNegOnes<V> extends 0 | 1
    ? number extends Product<S>
      ? unknown
      : number extends ProductSkipNegOne<V>
        ? unknown
...
type SwapDims<S extends Shape, I extends number, J extends number> = number extends I
  ? number[]
  : number extends J
    ? number[]
    : {
...
export type Transpose<S extends Shape, D0 extends number, D1 extends number> =
  IsDynamic<S> extends true ? number[] : SwapDims<S, NormalizeDim<S, D0>, NormalizeDim<S, D1>>;
...
export type Permute<S extends Shape, Order extends number[]> =
  IsDynamic<S> extends true
    ? number[]
    : {
...
type AllValidDims<S extends Shape, Ds extends number[]> = Ds extends [
  infer X extends number,
  ...infer Xs extends number[],
]
  ...
  IsDynamic<S> extends true
    ...
    : Order["length"] extends S["length"]
      ? AllValidDims<S, Order> extends true
        ? HasDup<NormalizeDims<S, Order> extends infer N extends number[] ? N : never> extends false
          ? unknown
...
type NormalizeUnsqueezeDim<S extends Shape, D extends number> = number extends D
  ? number
  : IsNegative<D> extends true
  ...
  IsDynamic<A> extends true
    ...
    : IsDynamic<B> extends true
      ...
      : NormalizeDim<A, D> extends infer I extends number
        ? number extends I
          ? number[]
          : CatDim<A, B, I> extends infer R extends number[]
            ? R
            : never
        : never;
      ...
      : A["length"] extends B["length"]
        ? IsValidDim<A, D> extends true
          ? NormalizeDim<A, D> extends infer I extends number
            ? number extends I
              ? unknown
              : EqualExceptAt<A, B, I> extends true
                ? unknown
...
type DimOpt<D extends number> = number extends D ? D | 1 : D extends 1 ? 1 : D | 1;

type MaskShape<S extends Shape> = S extends [infer X extends number, ...infer Xs extends number[]]
  ? DimOpt<X> extends infer D
    ? D extends number
      ? [D, ...MaskShape<Xs>]
      : never
    : never
...
export type BroadcastsInto<S extends Shape> =
  IsDynamic<S> extends true
    ? number[]
    : S extends [any, ...infer Xs extends number[]]
      ? MaskShape<S> | BroadcastsInto<Xs>
      : [];

export type InferShape<T, Depth extends 1[] = []> = Depth["length"] extends 12
  ? number[]
  : T extends number
    ? []
