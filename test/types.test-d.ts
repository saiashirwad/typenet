...
import { tensor, zeros, ones, randn, Tensor } from "../src/tensor.ts";
...
type _b2 = Expect<Equal<Broadcast<[8, 1, 6, 1], [7, 1, 5]>, [8, 7, 6, 5]>>;
type _b3 = Expect<Equal<CanBroadcast<[2, 3], [4]>, false>>;
type _b4 = Expect<Equal<Broadcast<[number, 3], [3]>, [number, 3]>>;

type _m1 = Expect<Equal<MatMul<[2, 3], [3, 4]>, [2, 4]>>;
type _m2 = Expect<Equal<MatMul<[10, 2, 3], [3, 4]>, [10, 2, 4]>>;
type _m3 = Expect<Equal<MatMul<[3], [3]>, []>>;
type _m4 = Expect<Equal<MatMul<[2, 3], [3]>, [2]>>;
type _m5 = Expect<Equal<MatMul<[3], [3, 4]>, [4]>>;
type _m6 = Expect<Equal<MatMul<[number, 784], [784, 128]>, [number, 128]>>;

type _v1 = Expect<Equal<ResolveView<[4, 6], [2, -1, 3]>, [2, 4, 3]>>;
type _t1 = Expect<Equal<Transpose<[2, 3, 4], 0, 2>, [4, 3, 2]>>;
type _t2 = Expect<Equal<Transpose<[2, 3, 4], -1, -2>, [2, 4, 3]>>;
type _p1 = Expect<Equal<Permute<[2, 3, 4], [2, 0, 1]>, [4, 2, 3]>>;
...
type _r2 = Expect<Equal<ReduceDim<[2, 3, 4], -1, true>, [2, 3, 1]>>;
...
type _i1 = Expect<Equal<InferShape<[[1, 2, 3], [4, 5, 6]]>, [2, 3]>>;
...
function _genericDims<N extends number, P extends TensorParams>(
  h: Tensor<[N, 24], P>,
  adj: Tensor<[N, N], any>,
): Tensor<[N, 16], P> {
  const w = zeros([24, 8]);
