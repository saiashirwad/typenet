...
function gradCheck(
  ...
  const make = () =>
    values.map((v, i) => {
      const t = Tensor.zeros(shapes[i]! as [number]).to("float64") as AnyTensor;
...
describe("autograd", () => {
  ...
  it("batched matmul gradients (numerical)", () => {
    gradCheck(
      (a, b) => a.matmul(b),
      [Array.from({ length: 12 }, (_, i) => i * 0.3 - 2), [0.5, -1, 2, 1.5]],
      [
        [3, 2, 2],
        [2, 2],
      ],
  ...
  it("activations gradients (numerical)", () => {
    gradCheck(
      (a) => a.relu().add(a.sigmoid()).add(a.tanh()).add(a.leakyRelu(0.2)),
      [[-2, -0.5, 0.5, 2]],
      [[4]],
    );
  });
  ...
  it("softmax / logSoftmax gradients (numerical)", () => {
    gradCheck((a) => a.view([2, 2]).softmax(1).pow(2), [[1, 2, 3, 4]], [[4]]);
    gradCheck((a) => a.view([2, 2]).logSoftmax(1).mul(0.5), [[1, 2, 3, 4]], [[4]]);
  });
  ...
  it("reshape / transpose / cat / stack gradients (numerical)", () => {
    ...
    gradCheck(
      (a, b) => Tensor.cat(a.view([1, 2]) as any, b.view([1, 2]) as any, 0).pow(2),
      [
        [1, 2],
        [3, 4],
      ],
  ...
  it("mean over dim gradients (numerical)", () => {
    gradCheck((a) => a.view([2, 3]).mean(1).pow(2), [[1, 2, 3, 4, 5, 6]], [[6]]);
  });
});
