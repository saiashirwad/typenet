...
import { Linear, Module, SGD, tensor, type Tensor, type TensorParams } from "../index.ts";

class XorNet extends Module {
  hidden = new Linear(2, 8);
  out = new Linear(8, 1);

  forward<B extends number, P extends TensorParams>(x: Tensor<[B, 2], P>) {
...
for (let epoch = 1; epoch <= 1500; epoch++) {
  ...
  if (epoch % 250 === 0)
    console.log(`epoch ${String(epoch).padStart(4)}  loss ${loss.item().toFixed(6)}`);
}
...
for (let i = 0; i < 4; i++) {
  const a = X.get(i, 0);
  const b = X.get(i, 1);
  console.log(`  ${a} xor ${b} -> ${final.get(i, 0).toFixed(4)} (target ${Y.get(i, 0)})`);
}
