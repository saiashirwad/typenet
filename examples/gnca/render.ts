...
export function renderSvg(
  ...
  const n =
    options.nodes ?? Math.floor(positions.length / dim)
  ...
  if (dim === 3)
    order.sort(
      (a, b) =>
        positions[a * dim + 2]! - positions[b * dim + 2]!
    )
  ...
  const panels = frames.map((frame, panel) => {
    ...
    const circles = order
      .map(i => {
        ...
        const cy =
          (1 - clamp01(positions[i * dim + 1]!)) * size
        const rgb = [0, 1, 2]
          .map(c =>
            Math.round(clamp01(frame[i * 4 + c]!) * 255)
          )
          .join(",")
        return (
  ...
  const width =
    frames.length * size + (frames.length - 1) * gap
  return (
