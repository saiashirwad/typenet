...
- `test`: a `macos-14` (Apple Silicon) matrix with two legs, `simd: [default,
  no_simd]`. The `no_simd` leg sets `TYPENET_NO_SIMD=1`; it is a no-op until
  the SIMD kernel work lands, wired now so the leg exists before the code
  that needs it. Both legs run the same steps: `pnpm install --frozen-lockfile`,
  `pnpm build:native`, then `pnpm test:ci`, which is
...
laptop-class VMs; they cannot hold a 3% no-regression clause, and a red bench
job here means nothing about a real regression. Its only purpose is to leave
a downloadable trail of `.jsonl` lines per commit. Every number quoted in
this repository's benchmark gates is measured on a dedicated, otherwise-idle
Apple M5 (4 performance cores + 6 efficiency cores, the same topology this
repository's benches print via
...
expression or schema errors. It cannot execute past "Set up job" locally,
because `act` has no `macos-14` runner image to begin with (it only ever
substitutes a Linux container for any `runs-on` label), so a full local
`act` run would not exercise anything macOS/Xcode/cargo-specific anyway.
`actionlint` (installed via `brew install actionlint`) reports zero problems
against `.github/workflows/ci.yml`. Both matrix legs were additionally
verified green by running their real commands directly on an Apple Silicon,
`macos-14`-equivalent machine: `pnpm install --frozen-lockfile`,
`pnpm build:native`, `pnpm test:ci`, and again with `TYPENET_NO_SIMD=1`. The
authoritative validation remains a pushed run on GitHub's actual `macos-14`
runners.
...
(matches the recorded baseline of "333 tests, 15 files, 18.6 s wall" within
run-to-run noise.)

Two `vite.config.ts` `test` options were tried and reverted; the file ships
unchanged:
...
- `isolate: false` alone: two runs at 20.08 s / 19.98 s, no improvement
  ...
  the machine's 10 cores regardless of per-file isolation, so removing
  isolation buys nothing.
- `isolate: false` + `poolOptions.threads.singleThread: true`: same ~20 s
  wall, but 1 test file fails (282/283 pass instead of 333/334);
  running every test file in one shared thread/module registry surfaces
  cross-file state pollution. Rejected on correctness grounds alone,
...
Pre-existing flake: `pnpm vitest run
